        ...
        <div className="p-6 max-w-6xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredData.map((city, index) => (
              <Link to={`/city/${city.location.name.toLowerCase()}`} key={index} className="bg-white hover:shadow-lg transition-shadow border border-blue-100 rounded-lg">
          ...
          <div className="mt-10">
            <h2 className="text-xl font-bold text-blue-900">Weather Data Visualization</h2>
            <BarChart width={600} height={300} data={filteredData}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="current.temperature" fill="#8884d8" />
              <Bar dataKey="current.humidity" fill="#82ca9d" />
            </BarChart>
          </div>
...
const CityDetail = ({ weatherData }) => {
  ...
  const city = weatherData.find(city => city.location.name.toLowerCase() === cityName);
  ...
  return (
    <div className="p-6 bg-white shadow-lg rounded-lg">
      <h1 className="text-2xl font-bold">{city.location.name}, {city.location.country}</h1>
      <div className="mt-4 space-y-2">
        <div className="flex justify-between">
          <span className="text-gray-700">Temperature:</span>
          <span className="font-bold">{city.current.temperature}°C</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-700">Humidity:</span>
          <span className="font-bold">{city.current.humidity}%</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-700">Wind Speed:</span>
          <span className="font-bold">{city.current.wind_speed} km/h</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-700">Cloud Cover:</span>
          <span className="font-bold">{city.current.cloudcover}%</span>
        </div>
      </div>
